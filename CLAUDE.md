...
**Meters are GDI, and monochrome by construction.** `draw_bar` and `draw_graph` share `fill_and_frame`, which lays the well down first and the frame last so a fill can never paint over its own edge. Frame, grid and fill are three `shade()` steps of the single `meter_color`, so they cannot clash however the user recolors them. The well beneath them is painted in `outline_color`, for the same reason the text is outlined in it: `SetLayeredWindowAttributes` applies one alpha to the whole window, so the desktop bleeds through every pixel equally, and a dark well is what keeps that bleed from washing the chart out. It cannot remove the bleed — only per-pixel alpha via `UpdateLayeredWindow` could, and that would replace the `WM_PAINT`/`BitBlt` model entirely. The chart is a `Polygon` anchored to the right edge and spaced by `capacity`, not by how many samples exist — that is what makes a half-full history scroll in at a steady rate instead of stretching. The grid snaps its cell to a divisor of the meter height, so the cells come out square and the rows fit exactly; a cell that does not divide the height leaves a one-pixel sliver of a row along the top.
