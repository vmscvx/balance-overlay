...
A layered always-on-top overlay window that polls API-credit balances and paints them as text, one line per provider, plus CPU, RAM and network load charts. Two threads, one shared state object.
...
**System metrics run on the UI thread.** `system.rs` is a handful of Win32 calls: `GlobalMemoryStatusEx` (whose `dwMemoryLoad` is already a percentage), `GetSystemTimes`, and `GetIfTable2`. CPU and network only exist as *differences* between two samples, so `Metrics` holds the previous readings alongside the `HISTORY_LEN` rings, and they render `--` for one tick while memory, being absolute, shows up immediately. All of it takes microseconds and never blocks, which is why a `WM_TIMER` at `SYSTEM_TICK_MS` does the sampling directly in `wnd_proc` — no thread, no async, no new dependency. A per-second cadence is the point: the same number sampled by the 60-second balance poll would be meaningless, and one tick is one column of the chart.

**Histories store raw values; the chart is what normalises.** `History` keeps `u64` samples and `normalized(full)` maps them onto the 0..100 the renderer draws. Storing percentages instead would freeze each sample against the scale that happened to be current when it arrived, and an autoscaled chart has to restate its *whole* history the moment a new peak lands. CPU and RAM pass `100`; the network rows pass their own peak, floored at `NET_SCALE_FLOOR` so an idle link does not amplify its own noise to full height. Because the chart then only knows 0..100, the real magnitude lives in `Row::Graph::value`, formatted with its unit by the module that knows what the number means.

**Which network interface.** `select_interface` takes the busiest adapter whose `InterfaceAndOperStatusFlags` claim both `HardwareInterface` and `ConnectorPresent`, which is what separates a real card from a VPN tunnel, a hypervisor switch or a TAP device. That is not cosmetic: a tunnel runs over the card it tunnels through, so counting both reports the same bytes twice. The `network_interface` config key matches on alias or description instead, which is how someone deliberately watches a tunnel. The previous counters are stored *with* the interface LUID, because a delta taken across two different interfaces is not a rate.
...
- `per_second` converts a per-tick byte delta using `SYSTEM_TICK_MS`. Changing the tick without it would silently relabel the rate.
